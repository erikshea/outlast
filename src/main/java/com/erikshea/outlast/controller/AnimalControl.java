...
import com.erikshea.outlast.animals.*;
import com.erikshea.outlast.util.FileUtil;
import com.erikshea.outlast.util.TextUtils;
import java.io.File;
import java.net.URI;
...
public class AnimalControl<T extends Animal> extends HBox {
    ...
    @FXML
    private Label animalNameInfo, animalTypeInfo, animalAge, animalSize;	// Animal info labels 
    @FXML
    private ProgressBar healthBar, energyBar;	// stat bars
    @FXML
    private ProgressIndicator ageIndicator, excrementIndicator;	// stat indicators (circular)
    @FXML
    private StackPane animalProfile;		// Contains portrait and overlays (masks...)

    @FXML
    private ImageView animalPortrait, animalMask;		// picture of animal and corresponding mask
    @FXML
    private ContextMenu reproduceMenu, seeFriendMenu;	// context menu for social actions
    @FXML
    private Slider colorSlider;						// slider for color adjustment
    ...
    private void initialize() {
        ...
        // load portrait
        Image portrait = new Image(FileUtil.getAssetsDir() + "images/species/" + animal.getType() + ".png");
        ...
        Image mask = new Image(FileUtil.getAssetsDir() + "images/species/" + animal.getType() + "_mask.png");
     ...
     * @param originator clicked node, in case a context menu needs to know
     * where to appear
     */
    ...
    /**
     * Loops through all actions and sets up -icons -hover out effect -method to
     * call on click, passing action name
     */
    private void setUpActionButtons() {
        ...
        for (Node n : actionButtons) {
            ...
            String newButtonStyle = "-fx-background-image: url(\"/assets/images/actions/" + animal.getType() + "/" + actionName + ".png\")"
                    + actionButton.getStyle();
            actionButton.setStyle(newButtonStyle);
            actionButton.setOnMouseEntered(e -> {
                actionButton.setOpacity(0.3);
            }); // reduce opacity on hover in
            actionButton.setOnMouseExited(e -> {
                actionButton.setOpacity(1);
            }); // restore opacity on hover out
            actionButton.setOnMousePressed(e -> {
                this.executeAction(actionName, actionButton);
    ...
    /**
     * Sets a reference to the main controller (for console, and to communicate
